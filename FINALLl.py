...
def reconstruct(time):    
      ...
      for i in range(0,len( st.session_state['sampled_signal_drawn']), 1):
        # signal=added_magnitude * np.sin(2 * np.pi*added_frequency*i*(1/freqsample))
        sum+=np.dot(st.session_state['sampled_signal_drawn'][i],np.sinc((time-i*1/st.session_state['freqsample'])/(1/st.session_state['freqsample'])))
      return sum 


def sampling(SampleFrequency):
    #   T=1/SampleFrequency    #sample time
      time2=np.linspace(0,1,SampleFrequency)
    #   nT=n*T   # x[n]=x(nT)
      st.session_state['sampled_signal_drawn']=np.zeros(len(st.session_state['time'])) 
      for i in range(len(time2)):  
        for item in st.session_state['table']:
           st.session_state['sampled_signal_drawn'][i] += item[0]*np.sin(2*np.pi*item[1]*time2[i])
      st.session_state['fig3']=pp.line(x=st.session_state['time'], y=st.session_state['signal_drawn'])
      st.session_state['fig3'].add_scatter(x=time2, y=st.session_state['sampled_signal_drawn'],mode='markers') 

      interpolation=reconstruct (st.session_state['time'])   
